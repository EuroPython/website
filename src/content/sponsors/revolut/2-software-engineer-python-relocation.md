---
title: "Software Engineer (Python) - Relocation to Poland, Spain, or UAE"
location: "Poland · Spain · UAE"
type:
level:
tags:
salary:

description: "
  This role offers the opportunity to [relocate to Poland, Spain, or the
  UAE](https://www.revolut.com/blog/post/relocation-campaign/). You'll receive
  support from Revolut throughout the entire process. Once you move, you'll have
  the flexibility to work full remotely or in a hybrid model in our offices in
  Krakow, Barcelona, Madrid, or Dubai.


  Our Technology team builds the systems and experiences that keep Revolut
  moving. From the infrastructure behind our innovative app to the features used
  by millions of people around the world, they bring sharp thinking, speed, and
  a focus on meaningful impact to everything they do.


  We're looking for a Backend Engineer to join a team that keeps frameworks lean
  and focuses on what matters: clean, maintainable code, shipped fast with TDD,
  DDD, and continuous integration and delivery.


  Our stack includes Python 3.11, Celery, FastAPI, Django, PostgreSQL, Redis,
  Airflow, and GCP.


  Up to shape what's next in finance? Let's get in touch.
  "

responsibilities:
  - "Building APIs and jobs and data pipelines, making sure they're properly
    designed and scaled according to business needs"
  - "Writing event consumers to build data models for new flows and processes"

requirements:
  - "5+ years of experience as a Software Engineer"
  - "3+ years of experience engineering with Python as your primary language"
  - "An academic background in STEM"
  - "Fluency in Python, SQL, and other OOPLs"
  - "Experience with API development and integration"
  - "A practical understanding of distributed systems"
  - "The ability to write concurrent code in IO/CPU bound situations"
  - "Experience with Docker, K8s, Ansible, Teamcity, monitoring, and alerting"

preferred:
  - "Experience with prototyping and sketching"
  - "Multiple side projects or open source contributions"
  - "Exposure to GCP"

benefits:
apply_link: "https://revolut.la/EuroPython2026_SoftwareEngineerPython_RelocationtoPolandSpainorUAE"
---
