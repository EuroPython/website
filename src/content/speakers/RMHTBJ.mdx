---
affiliation: Intel
avatar: https://program.europython.eu/media/avatars/michal.karzynski_8sqgZB4.jpg
code: RMHTBJ
homepage: https://michal.karzynski.pl/
mastodon: null
name: "Micha\u0142 Karzy\u0144ski"
slug: michal-karzynski
submissions:
  - contributing-to-onnx-how-you-can-improve-machine-learning-interoperability
twitter: "@postrational"
---

Michal works as a data scientist and software architect for Intel. He is also a
chairperson of the Operators group of the open source ONNX (Open Neural Network
Exchange) project.
